...
interface IwORKAN {
    function sORKANTowORKAN( uint amount ) external view returns ( uint );
    function wORKANTosORKAN( uint amount ) external view returns ( uint );
}
...
/**
 *  This contract allows Strudel seed investors and advisors to claim tokens.
...
contract InvestorClaimV2 is Ownable {
    ...
    struct Term {
        ...
        uint max; // maximum nominal ORKAN amount can claim
    }
    ...
    IERC20 immutable ORKAN; // claim token
    IERC20 immutable PRINCIPAL; // payment token

    ITreasury immutable treasury; // mints claim token
    IStaking immutable staking; // stake ORKAN for sORKAN

    address immutable DAO; // holds non-circulating supply
    IwORKAN immutable wORKAN; // tracks rebase-agnostic balance
    ...
    constructor( 
        ...
        require( _wORKAN != address(0) );
        wORKAN = IwORKAN( _wORKAN );
    ...
    /**
     *  @notice allows wallet to claim ORKAN
     *  @param _amount uint
     */
    function claim( uint _amount ) external {
        ORKAN.safeTransfer( msg.sender, _claim( _amount ) ); // send claimed to sender
    }

    /**
     *  @notice logic for claiming ORKAN
     *  @param _amount uint
     *  @return ToSend_ uint
     */
    function _claim( uint _amount ) internal returns ( uint ToSend_ ) {
        ...
        ToSend_ = treasury.deposit( _amount, address( PRINCIPAL ), 0 ); // deposit into treasury, receive ORKAN
        ...
        terms[ msg.sender ].wClaimed = terms[ msg.sender ].wClaimed.add( wORKAN.sORKANTowORKAN( ToSend_ ) );
    }
    ...
    /**
     *  @notice view ORKAN claimable for address. PRINCIPAL decimals (18).
    ...
    /**
     *  @notice view ORKAN claimed by address. ORKAN decimals (9).
     *  @param _address address
     *  @return uint
     */
    function claimed( address _address ) public view returns ( uint ) {
        return wORKAN.wORKANTosORKAN( terms[ _address ].wClaimed );
    }

    /**
     *  @notice view circulating supply of ORKAN
     *  @notice calculated as total supply minus DAO holdings
     *  @return uint
     */
    function circulatingSupply() public view returns ( uint ) {
        return ORKAN.totalSupply().sub( ORKAN.balanceOf( DAO ) );
    }
    ...
    function setTerms(address _address, uint _max, uint _rate, uint _hasClaimed ) external {
        ...
        if( terms[ _address ].max == 0 ) {
            terms[ _address ].wClaimed = wORKAN.sORKANTowORKAN( _hasClaimed );
        } 
