...
contract StrudelTreasury is Ownable {
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ORKAN decimals and divide by number of tokens for 1 ORKAN
            value_ = _amount.mul( 10 ** IERC20( ORKAN ).decimals() ).div( 10 ** IERC20( _token ).decimals() ).mul(10 ** 9).div(nativeFloorPrice[_token]);
    ...
    /**
        @notice returns ORKAN valuation of asset for the front-end to read because valueOf is a native function in javascript
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function _valueOf( address _token, uint _amount ) public view returns ( uint ) {
        return valueOf(_token, _amount);
    }

    /**
        @notice sets ORKAN valuation of asset
        @param _token address
        @param _value uint
     */
